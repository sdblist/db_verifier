...
Актуальная версия применима к PostgreSQL 12 и новее. Протестировано в версиях PostgreSQL с 12 по 18.
...
| fk1014 |             | duplicate FK attributes in source relation                          | error    | enable        | Дублируются атрибуты внешнего ключа в исходном отношении.                                                                            |
...
| i1012  |             | duplicate columns in index                                          | error    | enable        | В индексе дублируются колонки.                                                                                                       |
...
| n1015  |             | confusion in name of indexes                                        | warning  | enable        | Возможна путаница в наименованиях индексов одного отношения. Наименования опасно похожи.                                             |
...
| n1025  |             | confusion in name of constraint                                     | warning  | enable        | Возможна путаница в наименованиях ограничений одного отношения. Наименования опасно похожи.                                          |
| n1026  |             | unwanted characters in constraint name                              | notice   | enable        | Имя ограничения содержит нежелательные символы, такие как точки, пробелы и др.                                                       |
...
| n1040  |             | schema name reserved keyword                                        | warning  | enable        | Имя схемы совпадает с зарезервированным ключевым словом.                                                                             |
...
| pg0001 |             | wrong volatility marking for json_strip_nulls and jsonb_strip_nulls | error    | enable        | Некорректное значение provolatile для функций json_strip_nulls и jsonb_strip_nulls (см. замечания к PG 18.3).                        |
| pg0002 |             | PUBLIC has CREATE on public schema (PG>=15)                         | error    | enable        | У роли PUBLIC не отозвано разрешение CREATE для схемы public (см. замечания к PG 15.0).                                              |
