WITH
    ...
    check_description AS (
        ...
        FROM
            (VALUES
                ('no1001', null, 'The relationship does not have a unique key (a set of fields). This may cause issues when deleting records, during logical replication, and in other cases.'),
                ...
                ('fk1002', null, 'Foreign key uses columns that allow NULL values.'),
                ('fk1002', 'ru', 'Внешний ключ использует колонки, допускающие значение NULL.'),
                ('fk1007', null, 'Relation is not used in any foreign keys (may no longer be needed).'),
                ('fk1007', 'ru', 'Отношение не используется во внешних ключах (возможно оно больше не нужно).'),
                ('fk1010', null, 'FK are very similar (may be identical).'),
                ...
                ('c1001',  null, 'Constraint was not validated for all data (there may be records that violate the constraint).'),
                ('c1001',  'ru', 'Ограничение не проверено для всех данных (возможно присутствуют записи, нарушающие ограничение).'),
                ('i1001',  null, 'Indexes are very similar (may be identical).'),
                ('i1001',  'ru', 'Индексы очень похожи (возможно совпадают).'),
                ('i1002',  null, 'Index has bad signs.'),
                ('i1002',  'ru', 'Индекс имеет признаки проблем.'),
                ('i1003',  null, 'Unique and non-unique indexes are very similar (non-unique one might be redundant).'),
                ('i1003',  'ru', 'Уникальный и не уникальный индексы очень похожи (возможно не уникальный лишний).'),
                ('i1005',  null, 'Indexes are similar in terms of the set of fields (rough comparison).'),
                ('i1005',  'ru', 'Индексы похожи по набору полей (грубое сравнение).'),
                ('i1010',  null, 'B-tree index on a field with an array of values does not index the individual elements of the array (GIN index might be needed).'),
                ...
                ('n1015',  null, 'There may be confusion in the name of the indexes. The names are dangerously similar.'),
